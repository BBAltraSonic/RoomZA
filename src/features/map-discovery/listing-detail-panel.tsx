...
function TrueMonthlyCostCard({ listing }: { listing: ListingDetail }) {
  ...
  return (
    <div className="flex h-full flex-col bg-panel text-ink">
      ...
      <div className="border-b border-border p-4 sm:p-5">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {(() => {
                ...
                if (daysAgo === 0) return <StatusBadge tone="neutral">Listed today</StatusBadge>;
                if (daysAgo === 1) return <StatusBadge tone="neutral">Listed yesterday</StatusBadge>;
                if (daysAgo < 30) return <StatusBadge tone="neutral">Listed {daysAgo} days ago</StatusBadge>;
                return null;
              })()}
            </div>
            <h1 className="mt-2 line-clamp-2 text-xl font-semibold leading-tight tracking-normal text-ink sm:mt-3 sm:text-2xl">
              {listing.title}
            </h1>
            <p className="mt-1.5 flex items-center gap-1.5 text-sm text-muted-foreground sm:mt-2">
        ...
        <p className="mt-3 text-2xl font-semibold text-ink sm:mt-4">
      ...
      <div className="flex-1 space-y-5 overflow-y-auto p-4 scrollbar-hide sm:space-y-6 sm:p-5" onScroll={onScroll}>
        ...
        <TrueMonthlyCostCard listing={listing} />
        ...
        <div className="grid grid-cols-3 gap-2 sm:grid-cols-2 sm:gap-3">
          <div className="flex flex-col items-center rounded-xl border border-border bg-warm-surface px-2 py-3 text-center sm:flex-row sm:items-center sm:gap-3 sm:rounded-md sm:px-3 sm:py-2.5 sm:text-left">
            ...
            <div className="mt-1.5 sm:mt-0">
              <p className="text-sm font-semibold text-ink sm:text-sm">{listing.bedrooms}</p>
              <p className="text-[0.65rem] text-muted-foreground sm:text-xs">Bedrooms</p>
            </div>
          </div>
          <div className="flex flex-col items-center rounded-xl border border-border bg-warm-surface px-2 py-3 text-center sm:flex-row sm:items-center sm:gap-3 sm:rounded-md sm:px-3 sm:py-2.5 sm:text-left">
            <Bath className="size-5 text-forest sm:size-4" />
            <div className="mt-1.5 sm:mt-0">
              <p className="text-sm font-semibold text-ink sm:text-sm">{listing.bathrooms}</p>
              <p className="text-[0.65rem] text-muted-foreground sm:text-xs">Bathrooms</p>
            </div>
          </div>
          <div className="flex flex-col items-center rounded-xl border border-border bg-warm-surface px-2 py-3 text-center sm:flex-row sm:items-center sm:gap-3 sm:rounded-md sm:px-3 sm:py-2.5 sm:text-left">
            <Car className="size-5 text-forest sm:size-4" />
            <div className="mt-1.5 sm:mt-0">
              <p className="text-sm font-semibold text-ink sm:text-sm">{parkingTypeLabels[listing.parking_type as keyof typeof parkingTypeLabels] ?? listing.parking_type}</p>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button
            ...
            className="h-12 rounded-xl border-border bg-panel text-ink hover:bg-warm-surface disabled:opacity-50 sm:h-10 sm:rounded-md"
            variant="outline"
          >
