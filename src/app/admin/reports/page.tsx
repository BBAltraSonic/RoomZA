...
export default async function AdminReportsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <>
      <AdminHeader title="Moderation reports" description="Assign, investigate, and resolve listing, profile, message, or photo reports." action={context.membership.level === "owner" ? <Button render={<Link href={`/admin/exports/cases${exportQuery ? `?${exportQuery}` : ""}`} />} variant="outline">Export CSV</Button> : undefined} />
