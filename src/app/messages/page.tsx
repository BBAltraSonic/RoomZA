...
export default async function MessagesOverviewPage() {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-background">
      ...
      <section className="absolute inset-x-0 top-0 bottom-[calc(var(--mobile-bottom-nav-h)+var(--mobile-safe-bottom))] z-40 flex w-full flex-col overflow-hidden border-border bg-panel shadow-[var(--elevation-3)] sm:inset-y-0 sm:left-auto sm:right-0 sm:max-w-md sm:rounded-none sm:border-l">
        <div
          className="border-b border-border px-5 pb-4 pt-4 sm:pt-20"
        ...
        <div className="flex-1 overflow-y-auto p-4 pb-5 sm:pb-24">
                ...
                return (
                  <li key={convo.id}>
                    <Link
                      ...
                      className="group flex items-center gap-3 rounded-2xl border border-border bg-panel p-3.5 shadow-[var(--elevation-1)] transition-all hover:border-forest/30 hover:bg-warm-surface active:scale-[0.98] sm:rounded-lg sm:p-3 sm:active:scale-100"
                    >
                      <div className="relative size-14 shrink-0 overflow-hidden rounded-xl border border-border bg-muted sm:rounded-md">
                        {listingImage ? (
